...
public class AdminServiceImpl implements AdminService
{
    ...
    private ExtraInfoDao extraInfoDao = new ExtraInfoDaoImpl();
     ...
     * 添加一条额外记录(管理员填写)
     * @param name 借用人姓名
     * @param infos 管理员填写的额外信息
     * @return
     */

    public JSONObject addExtraInfo(String name,String infos,String d_no,String wechatID)
    {
        JSONObject info = new JSONObject();
        JSONArray errMsg = new JSONArray();
        int flag;
        Device device = deviceDao.getDeviceByNo(d_no);
        Admin admin = adminDao.getAdminByWechatID(wechatID);
        System.out.println();
        if(device == null) {
            flag = 0;
            errMsg.add("抱歉，不存在该仪器编号");
        }
        else if (!device.getA_no().equals(admin.getA_no())) {
            flag = 0;
            errMsg.add("抱歉，该仪器不在您的管辖范围内");
        }
        else {
            flag = extraInfoDao.addExtraInfo(name,infos,d_no);
            if(flag == 0) {
                errMsg.add("添加记录失败");
            }
        }
     ...
     * @return
     */
    public JSONObject setReturned(int ei_no)
    {
        JSONObject info = new JSONObject();
        JSONArray errMsg = new JSONArray();

        int flag = extraInfoDao.setReturnedByNo(ei_no);
        if(flag == 0) {
            errMsg.add("归还失败");
        }
    ...
    @Override
    public JSONObject getFinishedExtraRecord(String wechatID,int page,int count) {
        JSONObject info = new JSONObject();
        JSONArray errMsg = new JSONArray();

        Admin admin = adminDao.getAdminByWechatID(wechatID);
        String a_no = admin.getA_no();

        int flag = 1;
        List<ExtraInfo> list = extraInfoDao.getFinishedExtraRecord(a_no,page,count);
    ...
    @Override
    public JSONObject getUnfinishedExtraRecord(String wechatID,int page,int count) {
        JSONObject info = new JSONObject();
        JSONArray errMsg = new JSONArray();
        ...
        int flag = 1;
        List<ExtraInfo> list = extraInfoDao.getUnfinishedExtraRecord(a_no,page,count);
