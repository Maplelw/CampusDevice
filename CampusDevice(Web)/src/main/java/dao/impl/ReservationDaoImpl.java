...
public class ReservationDaoImpl implements ReservationDao
{
     ...
     */
    public List<Reservation> handleReservation(String a_no,int page,int count)
    {
        //初始化
        con = null;
        pStmt = null;
        ...
        {
            con = JDBCUtils.getConnection();
            sql = "SELECT r_no,d.d_no, d.d_name, d.d_model,d_save_site,COUNT(*) r_sum " +
                    "FROM reservation r, device d " +
                    "WHERE r.d_no = d.d_no " +
                    "AND d.a_no = ? AND r_state IN(0, 3) " +
                    "GROUP BY d.d_no, d.d_name, d.d_model " +
                    "ORDER BY r_sum DESC limit ?,?";
            pStmt = con.prepareStatement(sql);
            ...
            while (rs.next())
            {
                ...
                reservatioin.setR_no(rs.getInt("r_no"));
                reservatioin.setD_no(rs.getString("d_no"));
                reservatioin.setD_name(rs.getString("d_name"));
                reservatioin.setD_model(rs.getString("d_model"));
                reservatioin.setD_saveSite(rs.getString("d_save_site"));
    ...
    /*
     * @Description: 获取预约设备总数
     * @Param a_no
     * @Return: int
     */
    public int getReserveDeviceSum(String a_no)
    {
        //初始化
        con = null;
        pStmt = null;
        ...
        {
            con = JDBCUtils.getConnection();
            sql = "SELECT COUNT(DISTINCT d.d_no)\n" +
                    "FROM reservation r, device d \n" +
                    "WHERE r.d_no = d.d_no \n" +
                    "AND d.a_no = ? AND r_state IN(0, 3)";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Reservation> handleReservationDetail(String d_no)
    {
        ...
        {
            con = JDBCUtils.getConnection();
            sql = "SELECT r_no,r_state, u.u_no, u_name, u_type, r_no, r_start_date, r_return_date, r_reservation_date, u_credit_grade " +
    ...
    /*
     * @Description: 获取某一个设备的预约队列按照信用分从高到低排序
     * @Param deviceNo
     * @Return: java.util.List<bean.Reservation>
     */
    ...
    /*
     * @Description: 获取某一个设备的预约队列按照借用起始日期从早到晚排序
     * @Param deviceNo
     * @Return: java.util.List<bean.Reservation>
     */
